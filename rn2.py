...
Original file is located at
    https://colab.research.google.com/drive/1Vt93mKdEtIrU25A_oHVNirsriIApKQuj
"""
...
from optimisers import AdamOptimiser, SGDOptimiser, SGDMomentumOptimiser, NeuronLeakOptimiser,  NetworkLeakOptimiser, NetworkCostOptimiser, SynapseIntegratorOptimiser, NeuronIntegratorOptimiser, LayerIntegratorOptimiser,  NetworkIntegratorOptimiser, SynapseLeakOptimiser ,LayerLeakOptimiser 
...
class Experiment:
  ...
  def intd(self):
        ...
        eps = eps,
        device=self.device)
        '''optimiser = SynapseIntegratorOptimiser(
             weight_sizes=[model.classification_layers[i].weight.shape for i in range(len(self.model.classification_layers))],
             bias_sizes=[model.classification_layers[i].bias.shape for i in range(len(self.model.classification_layers))],
             eps = eps,
             device = self.device
  ...
  def run(self):
    ...
    for seed in range(self.num_seeds + 1):
      ...
      variance_of_test.append(var)
      print(np.shape(variance_of_test))
      variance_of_acc.append(var2)
    ...
    for i in range (20):
      ...
      save2 = []
      for j in range(4):
        save1.append(variance_of_test[j][i])
        save2.append(variance_of_acc[j][i])

      test_variance = np.var(save1)
      test_accuracy = np.var(save2)
      var_for_test_error.append(test_variance)
      var_for_test_acc.append(test_accuracy)
