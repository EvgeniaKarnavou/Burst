...
Original file is located at
    https://colab.research.google.com/drive/1JTDaFwXRYDQ0N8ZIplC7ghsFH1rXw4wR
"""
...
class SynapseLeakOptimiser:
    ...
    def update_buffers(self, weight_grads, bias_grads):
        for weight_G_buffer, bias_G_buffer, weight_grad, bias_grad in zip(self.wight_G_buffers, self.bias_G_buffers, weight_grads, bias_grads):

            weight_G_buffer.mul_(self.decay_constant).add_(weight_grad ** 2, alpha=1 - self.decay_constant)
            bias_G_buffer.mul_(self.decay_constant).add_(bias_grad ** 2, alpha=1 - self.decay_constant)

    def compute_updates(self, lrs, weight_grads, bias_grads):
        weight_updates = []
        bias_updates = []

        for lr, weight_G_buffer, bias_G_buffer, weight_grad, bias_grad in zip(lrs, self.wight_G_buffers, self.bias_G_buffers, weight_grads, bias_grads):
...
class SynapseIntegratorOptimiser:
    ...
    def compute_updates(self, lrs, weight_grads, bias_grads):
        ...
        for lr, weight_G_buffer, bias_G_buffer, weight_grad, bias_grad in zip(lrs, self.weight_G_buffers, self.bias_G_buffers, weight_grads, bias_grads):
            ...
            weight_update = -lr * weight_grad / weight_denom #change in lr
            bias_update = -lr * bias_grad / bias_denom.reshape(-1)
