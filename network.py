...
Original file is located at
    https://colab.research.google.com/drive/1diQ5fjJy20k-Q9NdCR9KJTM8VlLeD1KX
"""
...
#!ls
from layermethods import Flatten
from layers import BurstCCNHiddenLayer, BurstCCNOutputLayer, DLBurstCCNHiddenLayer
from optimisers import AdamOptimiser, SGDOptimiser, SGDMomentumOptimiser, NeuronLeakOptimiser, \
...
class BurstCCN(nn.Module):
    def __init__(self, n_inputs, n_outputs, p_baseline, n_hidden_layers, n_hidden_units, Y_mode, Q_mode, Y_scale,
        ...
        assert Q_mode in ['tied', 'symmetric_init', 'random_init']
        self.Q_mode = Q_mode
        ...
        self.out = nn.Sequential(*(self.feature_layers + self.classification_layers)) #what does this do, ask will
    ...
    def backward(self, target):
        ...
        for i in range(len(self.classification_layers) - 2, -1, -1): #4 epanalhpseis: apo 3, 2, 1, 0
            burst_rate, event_rate, feedback_bp, feedback_fa = self.classification_layers[i].backward(burst_rate,
                                                                                                      event_rate,
                                                                                                      feedback_bp,
                                                                                                      feedback_fa)

    def update_weights(self, lrs, lrs_Y, lrs_Q, optimiser, momentum=0, weight_decay=0, global_cost=None):
        ...
        for i in range(len(self.classification_layers)):
            ...
            self.classification_layers[i].update_secondary_weights(lr_Y=lrs_Y[i], lr_Q=lrs_Q[i], momentum=momentum,
    ...
    def get_activations(self):
      activations = {}
      for l, layer in enumerate(self.classification_layers):
        activations[l] = layer.e
      return activations
    ...
    def log_layer_states(self):
        ...
        for i in range(1, len(self.classification_layers) - 1):
            ...
            wandb.log({f"hidden{i}.event_rate": layer.e.flatten().cpu().numpy(),
                       f"hidden{i}.apical": layer.apic.flatten().cpu().numpy(),
                       f"hidden{i}.burst_prob": layer.p_t.flatten().cpu().numpy(),
                       f"hidden{i}.burst_rate": layer.b_t.flatten().cpu().numpy()}, commit=False)
    ...
    def log_layer_statesJen(self):
        ...
        # Log output
        output_layer = self.classification_layers[-1]
        wandb.log({"output.event_rate": output_layer.e.flatten().cpu().numpy(),
                   "output.burst_prob": output_layer.p_t.flatten().cpu().numpy(),
                   "output.burst_rate": output_layer.b_t.flatten().cpu().numpy()}, commit=False)
        wanb.log({'output_b':output_layer.b.flatten().cpu().numpy()})
